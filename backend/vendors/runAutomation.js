...
// SHARED BofA: 10 browsers for whichever source needs them (reduced to avoid rate limiting)
const BOFA_TOTAL_CONCURRENCY = Number(process.env.BOFA_TOTAL_CONCURRENCY || 10);
...
// Mutex to prevent race conditions when requesting BofA access
let bofaLockMutex = false;
let bofaLockQueue = [];

async function acquireBofaMutex() {
  return new Promise(resolve => {
    if (!bofaLockMutex) {
      bofaLockMutex = true;
      resolve();
    } else {
      bofaLockQueue.push(resolve);
    }
  });
}

function releaseBofaMutex() {
  if (bofaLockQueue.length > 0) {
    const next = bofaLockQueue.shift();
    next();
  } else {
    bofaLockMutex = false;
  }
...
async function runPrivyWithBofA() {
  ...
  try {
    ...
    while (!control.abort) {
      ...
      log.info(`PRIVY: Starting count: ${startCount}, calling runAutomation...`);

      resetBatchCounter();
      try {
        await runAutomation(new Set(['privy']));
      } catch (privyRunError) {
        log.error(`PRIVY: runAutomation failed!`, { error: privyRunError?.message || String(privyRunError) });
      }
