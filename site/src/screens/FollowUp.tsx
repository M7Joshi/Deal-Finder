  ...
  return (
    <div style={{ padding: isMobile ? 12 : 24 }}>
      ...
      <p style={{ color: '#6b7280', marginBottom: 16, fontSize: 14 }}>
        Deals that need follow-up. Add notes, set follow-up dates, or move to Email Sent or Deal Status.
      </p>
                ...
                <tr key={deal._id} style={{ background: i % 2 === 0 ? '#fff' : '#f9fafb' }}>
                  ...
                  <td style={{ padding: cellPadding, fontSize: cellFontSize }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span style={{ color: overdue ? '#dc2626' : '#374151', fontWeight: overdue ? 600 : 400, fontSize: 11 }}>
                        {formatDate(deal.followUpDate)}
                      </span>
                      {overdue && <Chip label="!" size="small" color="error" sx={{ fontSize: 9, height: 18 }} />}
                      <Button
                        size="small"
                        variant="text"
                        onClick={() => {
      ...
      <Dialog open={moveDialog.open} onClose={() => setMoveDialog({ open: false, deal: null, toStage: '' })} maxWidth="sm" fullWidth>
        <DialogTitle>
          Move to {moveDialog.toStage === 'email_sent' ? 'Email Sent' : 'Deal Status'}
        </DialogTitle>
        <DialogContent>
